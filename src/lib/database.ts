...
export async function initializeDatabase(): Promise<Low<DatabaseSchema>> {
  ...
  const adapter = new JSONFile<DatabaseSchema>(getDatabasePath());
  db = new Low(adapter, { products: [] });

  // Read existing data or create initial data
  await db.read();

  // If database is empty, populate with initial data
  if (!db.data.products || db.data.products.length === 0) {
    db.data.products = getInitialProducts();
    await db.write();
  }
...
// Generate slug function (consistent with existing logic)
...
// Initial products data (12 existing + 20 new)
